var clipboardCleaner = (function () {
    function clipboardCleaner(currentDocument, callback, env) {
        ...
        function keyEventRouter(args) {
            if (temp.CleanerState)
                temp.keydownHandler(args);
        }
    }
    clipboardCleaner.prototype.keydownHandler = function (e, state) {
        if (state === void 0) { state = false; }
        if ((this.isIE() && e.ctrlKey && !e.shiftKey && e.keyCode == 86 && this.flagState) || state) {
            this.flagState = false;
            this.setContent();
        }
    };
    clipboardCleaner.prototype.setContent = function () {
        var temp = this;
        ...
        if (window.getSelection) {
            ...
            if (selection.rangeCount > 0) {
                ...
                this.updateRange(tempText, 0, tempText.textContent.length);
                this.content.focus();
                this.syncLoad = false;
            }
        }
    };
    clipboardCleaner.prototype.InsertContent = function (args) {
        ...
        this.finalCleanup(this.Container);
        ...
        if (this.isIE() && args) {
            ...
            for (var gt = 0; gt < elemColl.length; gt++)
                this.insertAfter(this.content, elemColl[gt]);
            this.content.removeNode(true);
        }
        else
            this.contentPositionHandler();
        ...
        this.updateRange(content, 0, content.textContent.length);
    };
    clipboardCleaner.prototype.insertAfter = function (elem, newElem) {
        if (elem.nextSibling) {
    ...
    clipboardCleaner.prototype.findValidListType = function (elem) {
        if (elem.nodeType == 3 && elem.textContent.trim().length > 0) {
            this.listContents.push(elem.textContent);
        }
        if (elem = elem.firstChild) {
            do {
                this.findValidListType(elem);
            } while (elem = elem.nextSibling);
        }
    };
    clipboardCleaner.prototype.createNodeCollection = function (parentNodes) {
    ...
    clipboardCleaner.prototype.contentPositionHandler = function () {
        ...
        if (selection.rangeCount > 0) {
            ...
            if (range.startContainer == range.endContainer) {
                ...
                if (elem.nodeName.toLowerCase() != "body") {
                    ...
                    nodeSet = this.createNodeCollection(parentNodes);
                    if (elem)
                        if (elem.nextSibling)
                    ...
                    this.insertAfter(this.Container.parentElement, this.Container);
                }
    ...
    clipboardCleaner.prototype.updateRange = function (node, sOffset, eOffset) {
    ...
    clipboardCleaner.prototype.pasteHandler = function (args) {
        this.clipboardData = { plain: null, rtf: null, html: null };
        this.syncLoad = false;
        if (this.isIE())
            this.extractClipboardIE(args);
        else {
            this.fillClipboard(args);
            this.extractClipboardNonIE(args);
        }
    };
    clipboardCleaner.prototype.isIE = function () {
        return navigator.userAgent.match(/Trident|msie/i) != null;
    };
    clipboardCleaner.prototype.extractClipboardNonIE = function (args) {
        ...
        else if (args && args.clipboardData && args.clipboardData.files.length) {
    ...
    clipboardCleaner.prototype.extractClipboardIE = function (args) {
        this.contentUpdater();
    };
    clipboardCleaner.prototype.contentUpdater = function () {
        var temp = this;
        setTimeout(function () {
            if (temp.content)
    ...
    clipboardCleaner.prototype.fillClipboard = function (args) {
    ...
    clipboardCleaner.prototype.processContent = function (args) {
        ...
        if (patern.test(this.htmlContent)) {
            ...
            if (this.filterOptions.cleanElements) {
                this.blockquoteConvert();
                this.CleanME(elm);
                this.flush();
            }
            if (this.filterOptions.cleanCSS) {
                this.CleanStyles(elm);
                this.flush(false);
            }
            this.Container = elm;
            this.InsertContent();
            ...
            this.Container = elm;
            this.InsertContent(!nonBlock);
        }
    };
    clipboardCleaner.prototype.flush = function (check) {
    ...
    clipboardCleaner.prototype.MakeConvertion = function () {
        ...
        for (var index = 0; index < this.Collection.length; index++) {
            if ((this.Collection[index].level == 1) && listCount == 0) {
                if (this.Collection[index].content) {
                    ...
                    temp.style.listStyle = this.getlistStyle(this.Collection[index].type, this.Collection[index].level);
                }
            }
                ...
                if (prevList.parentElement.tagName.toLowerCase() == this.Collection[index].type) {
                    ...
                    this.insertAfter(prevList.parentElement, temp);
                    prevList = this.currentDocument.createElement("li");
                    prevList.appendChild(this.currentDocument.createTextNode(this.Collection[index].content.textContent));
                    temp.appendChild(prevList);
                    temp.setAttribute("level", this.Collection[index].level.toString());
    ...
    clipboardCleaner.prototype.listConverter = function () {
        ...
        for (var index = 0; index < this.listNodes.length; index++) {
            ...
            if (content && content.indexOf("level") != -1)
                ...
                level = 1;
            this.listContents = [];
            this.findValidListType(this.listNodes[index]);
    ...
    clipboardCleaner.prototype.blockquoteConvert = function () {
        var temp, elemColl = [];
    ...
    clipboardCleaner.prototype.cleanup = function (node) {
    ...
    clipboardCleaner.prototype.finalCleanup = function (node) {
        ...
        for (var index = 0; index < node.childNodes.length; index++) {
            if (node.childNodes[index] && node.childNodes[index].nodeType != 3
                ...
                && node.childNodes[index].innerText
                && (node.childNodes[index].innerText.trim() == ""))
                elemCollection.push(node.childNodes[index]);
        }
        while (temp = elemCollection.shift())
            this.remove(temp);
    };
    clipboardCleaner.prototype.getlistStyle = function (listType, level) {
    ...
    clipboardCleaner.prototype.CleanME = function (elem) {
        var temp;
        if (elem.nodeType == 3) {
    ...
    clipboardCleaner.prototype.CleanStyles = function (elem) {
        var temp;
        if (elem && elem.nodeType != 3) {
            this.cleanCSSClass(elem);
            this.cleanCSSStyle(elem);
        ...
        if (elem = elem.firstChild)
            do {
                this.CleanStyles(elem);
            } while (elem = elem.nextSibling);
    };
    clipboardCleaner.prototype.cleanCSSClass = function (elem) {
    ...
    clipboardCleaner.prototype.cleanCSSStyle = function (elem) {
    ...
    clipboardCleaner.prototype.remove = function (elem) {
        if (elem)
            if (elem.remove)
                elem.remove();
