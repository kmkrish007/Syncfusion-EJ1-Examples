(function ($, ej, undefined) {
    ...
    ej.ganttChartFeatures.timescale = {
        ...
        refreshChartHeader: function (startdate, enddate) {
            ...
            else if (scheduleMode == "year" || scheduleMode == "quarterYear" || scheduleMode == "halfYear") {
            ...
            else if (scheduleMode == "quarterYear" || scheduleMode == "halfYear") {
                proxy._createScheduleQuarterlyTemplate();
            }
            ...
            var updatedScheduleMonth = scheduleMode == "quarterYear" || scheduleMode == "halfYear" ? this._getQuarterHalfYearData(scheduleMonths, scheduleMode) : scheduleMonths;
            if (scheduleMode == "week")
                $tr = ej.buildTag("tr", $.render[this._id + "_CustomTemplate1"](proxy.model.scheduleWeeks), {}, {});
            if (scheduleMode == "year" || scheduleMode == "quarterYear" || scheduleMode == "halfYear")
                $tr = ej.buildTag("tr", $.render[this._id + "ScheduleMonthTemplate"](updatedScheduleMonth), {}, {});
        ...
        _createScheduleYearTemplate: function () {
            ...
            td = "<div class='e-schedule-week-headercell-content' style='margin:auto;overflow:hidden;white-space:nowrap;width:{{:~_" + proxy._id + "getYearWidth(#data)-1}}px;box-sizing:border-box;-moz-box-sizing:border-box;'>"
        ...
        //Template to quarterYear view
        _createScheduleQuarterlyTemplate: function () {
            var proxy = this, td, model = proxy.model;
            var parentTr = "<th  class='e-schedule-day-headercell' " +
                "style='-moz-box-sizing:border-box;box-sizing:border-box;width:{{:width}}px;'>" +
        ...
        _getMonthWidthForQuarterYear: function (data) {
            var proxy = this, quarterWidth = 0, length = data.length;
            for (var i = 0 ; i < length ; i++) {
                var date = new Date(data[i]),
                   monthLastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0),
                   beginDate = date.getDate(),
                   lastDate = monthLastDay.getDate();
                quarterWidth += this.model.perDayWidth * (beginDate != 1 ? (lastDate - date.getDate()) : lastDate);
            }
            return quarterWidth;
        },
        ...
        _getQuarterHalfYearData: function (scheduleMonths, scheduleMode) {
            var quarterlyData = [], quarterValue, halfValue, length = scheduleMonths.length, scheduleValue, value,
            ganttInstance=this.model.ganttInstance;
            for (var i = 0 ; i < length; i++) {
                var data = [],
                    quarterSchedule = {}, scheduleMonth = scheduleMonths[i].getMonth();
                if ((scheduleMonth / 3) < 1) {
                    quarterValue = 1;
                    halfValue = 1;
                }
                else if ((scheduleMonth / 3) < 2) {
                    quarterValue = 2;
                    halfValue = 1;
                }
                else if ((scheduleMonth / 3) < 3) {
                    quarterValue = 3;
                    halfValue = 2;
                }
                else if ((scheduleMonth / 3) < 4) {
                    quarterValue = 4;
                    halfValue = 2;
                }
                if (scheduleMonth % 3 == 0) {
                    data.push(scheduleMonths[i]);
                    data.push(scheduleMonths[i + 1]);
                    data.push(scheduleMonths[i + 2]);
                    i = i + 2;
                }
                else if (scheduleMonth % 3 == 1) {
                    data.push(scheduleMonths[i]);
                    data.push(scheduleMonths[i + 1]);
                    i = i + 1;
                }
                ...
                if (scheduleMode == "halfYear" && (scheduleMonth == 0 || scheduleMonth == 1 || scheduleMonth == 2 
                    || scheduleMonth == 6 || scheduleMonth == 7 || scheduleMonth == 8)) {
                    data.push(scheduleMonths[i + 1]);
                    data.push(scheduleMonths[i + 2]);
                    data.push(scheduleMonths[i + 3]);
                    i = i + 3;
                }
                quarterSchedule["width"] = this._getMonthWidthForQuarterYear(data);
                 scheduleValue = scheduleMode == "halfYear" ? "H" + halfValue : "Q" + quarterValue;
                switch (scheduleValue) {
                    case "Q1":
                        value = ganttInstance._quarterYearTexts["first"];
                        break;
                    case "Q2":
                        value = ganttInstance._quarterYearTexts["second"];
                        break;
                    case "Q3":
                        value = ganttInstance._quarterYearTexts["third"];
                        break;
                    case "Q4":
                        value = ganttInstance._quarterYearTexts["fourth"];
                        break;
                    case "H1":
                        value = ganttInstance._halfYearTexts["first"];
                        break;
                    case "H2":
                        value = ganttInstance._halfYearTexts["second"];
                        break;
                }
                quarterSchedule["mode"] = value;
                quarterlyData.push(quarterSchedule);
            }
        ...
        _createSchedule: function (startdate, enddate) {
            ...
            proxy._createMinuteTemplate();
            proxy._createScheduleHourTemplate();
            if (scheduleMode == "quarterYear" || scheduleMode == "halfYear") {
                proxy._createScheduleQuarterlyTemplate();
            }
            $table = ej.buildTag("table.e-schedule-headerrow-day e-zerospacing", "",
                {
