    ...
    ej.Pivot = {
       ...
       addReportItem: function (dataSource, args) {
            if (!args.isMeasuresDropped) {
                ...
                if (ej.isNullOrUndefined(reportItem)) reportItem = { fieldName: args.droppedFieldName, fieldCaption: args.droppedFieldCaption, format: args.droppedFieldFormat, formatString: args.droppedFieldFormatString, showSubTotal: args.droppedFieldShowSubTotal, expression: args.droppedExpression, hierarchyUniqueName: args.droppedHierarchyUniqueName, summaryType: args.summaryType};
        ...
        _updateValueSortingIndex: function(msg,ctrlObj){
            if (msg.SortColIndex != undefined) {
                ...
                var valueSorting = $.map(msg, function (obj, index) { if (obj != null) var keyString = obj.Key; if (keyString != null && keyString != undefined && keyString == "SortColIndex") return obj; });
                if (valueSorting.length > 0) {
                    msg.splice($.map(msg, function (obj, index) { var keyString = obj.Key; if (keyString == "SortColIndex") return index; }), 1);
            ...
            _memberSortBtnClick: function (args) {
                ...
                if (!ej.isNullOrUndefined(treeView) && treeView.length > 0) {
                    ...
                    if ((this.model.enableMemberEditorPaging || (!ej.isNullOrUndefined(this.model.pivotControl) && this.model.pivotControl.model.enableMemberEditorPaging))) {
                        if (drilledData.length == 0)
                            sortAll = ej.DataManager(sortAll).executeLocal(ej.Query().page(this._memberPageSettings.currentMemeberPage, ((this.model.memberEditorPageSize || this.model.pivotControl.model.memberEditorPageSize) + (this._memberPageSettings.currentMemeberPage == 1 ? 1:0))));
                    }
       ...
       _contextMenuOpen: function (args, ctrlObj) {
            ...
            if (mode == ej.Pivot.AnalysisMode.Olap) {
             ...
             var pivotclient = ctrlObj.model.pivotControl;
             if (!ej.isNullOrUndefined(pivotclient) && pivotclient.model.analysisMode == ej.Pivot.AnalysisMode.Pivot) {
                 if ($(args.target).parents(".e-schemaValue").length == 0)
                 menuObj.disableItem(ctrlObj._getLocalizedLabels("SummarizeValueBy"));
            }
        },	
        ...
        _updateSearchFilterSelection: function (args, treeElement, ctrlObj) {
            var extendedData = {}, filterData = {};
            if (args.id == "SearchFilterSelection") {
                ...
                if (args.type == "nodeCheck") {
                    ...
                    if (ctrlObj._isOptionSearch) {
                        ...
                        if (ctrlObj.model.enableMemberEditorPaging) {
                            extendedData = ctrlObj._currentFilterList;
                            $(extendedData).each(function (index, item) {                               
                             filterData[item.id] = item; });
                            }
                        $(ctrlObj._editorTreeData).each(function (index, item) {
                            for (var i = 0; i < searchElement.length; i++) {
                                if (searchElement[i].id != item.id) {
                                    if (!ej.isNullOrUndefined(ctrlObj._currentFilterList)) {
                                       if (ctrlObj.model.enableMemberEditorPaging && !ej.isNullOrUndefined(filterData) && filterData[$(item).attr("id")])
                                          {
                                          item.checkedStatus = filterData[$(item).attr("id")].checkedStatus;
                                         }
                                       else if (ctrlObj._currentFilterList[$(item).attr("id")])
                                          item.checkedStatus = ctrlObj._currentFilterList[$(item).attr("id")].checkedStatus;
                                     }
                                }
                                else {
            ...
            setTimeout(function () {
                ...
                if (searchVal != "") {
                    ...
                    if (searchTreeNodes.length > 0) {
                        ctrlObj._isSelectSearchFilter = ctrlObj.model.enableCheckStateOnSearch;
                ...
                var args = { id: "SearchFilterSelection", type: ctrlObj._isSelectSearchFilter ? "nodeCheck" : "nodeUncheck" };
                var treeElement = ctrlObj.element.find(".e-editorTreeView");
                ej.Pivot._updateSearchFilterSelection(args, treeElement, ctrlObj);
                ctrlObj._waitingPopup.hide();
...
//            ctrlObj._memberTreeObj.model.beforeExpand = ej.proxy(ctrlObj._beforeNodeExpand, ctrlObj);
        ...
        _separateAllMember: function (ctrlObj) {
            ...
            if (isClientServer) {
                ...
                ctrlObj.element.find(".allMember").ejCheckBox({ size: "small", width: "40px", checked: ctrlObj.element.find(".e-dialogOKBtn").attr("disabled") == "disabled" ? false : true, change: ej.proxy(ctrlObj._nodeCheckChanges, ctrlObj) });
            }
        ...
        loadMeasureElements: function (customArgs, data) {
            ...
            var treeViewData, newTreeData, newMeasures = [], sortedCaption = [], sortedCaptionList = [], measures = [], sortedMeasures = [], measureElements;
            var isMeasureSorting = contObj.pluginName == "ejPivotClient" && (contObj.model.sortCubeMeasures == ej.PivotAnalysis.SortOrder.Ascending || contObj.model.sortCubeMeasures == ej.PivotAnalysis.SortOrder.Descending) && contObj.model.sortCubeMeasures != ej.PivotAnalysis.SortOrder.None;
            if (isMeasureSorting) {
                sortType = contObj.model.sortCubeMeasures == ej.PivotAnalysis.SortOrder.Ascending ? "ascending" : "descending";
                for (var i = 0; i < $(data).find("row").length; i++) {
                    sortedCaption.push({ "name": $(data).find("row:eq(" + i + ") MEASURE_CAPTION").text() });
                }
                newTreeData = ej.DataManager(sortedCaption).executeLocal(ej.Query().sortBy("name", sortType));
                $.each(newTreeData, function (index, value) {
                    for (var j = 0; j < $(data).find("row").length; j++) {
                        if ((value.name == $(data).find("row:eq(" + j + ") MEASURE_CAPTION").text())) {
                            sortedCaptionList.push($(data).find("row:eq(" + j + ")"));
                        }

                    }
                });
                for (var i = 0; i < sortedCaptionList.length; i++) {
                    if (!($.inArray($(sortedCaptionList[i]).children("MEASUREGROUP_NAME").text(), newMeasures) >= 0)) {
                        newMeasures.push($(sortedCaptionList[i]).children("MEASUREGROUP_NAME").text());
                        measures.push({ "name": $(sortedCaptionList[i]).children("MEASUREGROUP_NAME").text() });
                    }
                }
                treeViewData = ej.DataManager(measures).executeLocal(ej.Query().sortBy("name", sortType));

                $.each(treeViewData, function (index, value) {
                    for (var j = 0; j < sortedCaptionList.length; j++) {
                        if ((value.name == $(sortedCaptionList[j]).children("MEASUREGROUP_NAME").text())) {
                            sortedMeasures.push($(sortedCaptionList[j]));
                        }
                    }
                });
                measureElements = sortedMeasures;
            }
            else
               measureElements = $(data).find("row");
            for (var i = 0; i < measureElements.length; i++) {
                var element = $(measureElements[i]), measureGRPName = element.children("MEASUREGROUP_NAME").text(), measureUQName = element.find("MEASURE_UNIQUE_NAME").text();
        ...
        _drillThroughCellClick: function (args, gridObj) {
            ...
            if (gridObj.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode) {
                ...
                var columnHeaderCount = $("#" + gridObj._id).find("tbody").find('[data-p="' + parseInt(cellPos.split(",")[0]) + "," + parseInt(cellPos.split(",")[1]) + '"]').closest('tbody').prev().children('tr').length;
        ...
        _getFilterStateMember: function (olapClientMode, members, item, controlObj, filterState) {
            if (item.filterItems.filterType == "include") {
                if (item.filterItems.values.length == 1) {
                    if (ej.isNullOrUndefined(controlObj._fieldSelectedMembers[item.fieldName.toLowerCase()]) && olapClientMode) {
                        var mdxQuery = "select {" + item.filterItems.values[0] + "}" + "dimension properties CHILDREN_CARDINALITY on 0 from [" + $.trim(controlObj.model.dataSource.cube) + "]";
                        var xmla = ej.olap._mdxParser.getSoapMsg(mdxQuery, controlObj.model.dataSource.data, controlObj.model.dataSource.catalog);
                        var conStr = ej.olap.base._getConnectionInfo(controlObj.model.dataSource.data);
                        this.doAjaxPost("POST", conStr.url, { XMLA: xmla }, ej.proxy(this._getInitialSlicerName, this), null, { controlObject: controlObj, action: "fetchMembers" });
                        filterState = controlObj._slicerName;
                    }
        ...
        _getFilterState: function (olapClientMode, members, item, controlObj) {
            ...
            if (controlObj.model.operationalMode == ej.Pivot.OperationalMode.ClientMode) {
                if (olapClientMode) {
                    ...
                    filterState = this._getFilterStateMember(olapClientMode, members, item, controlObj, filterState);
                }
                else
                    filterState = this._getFilterStateMember(olapClientMode, members, item, controlObj, filterState);
            }
                ...
                if (controlObj.model.analysisMode == ej.Pivot.AnalysisMode.Pivot) {
                    ...
                    else if (!ej.isNullOrUndefined(controlObj._clientReportCollection)) {
                        for (var i = 0; i < controlObj._clientReportCollection.length; i++) {
                            if (!ej.isNullOrUndefined(controlObj._clientReportCollection[i]._fieldSelectedMembers) && ((controlObj._currentRecordName == "" && controlObj.element.find("#reportList") != undefined && controlObj.element.find("#reportList")[0].value == controlObj._clientReportCollection[i].name ? true : false) || controlObj._currentRecordName != "" && controlObj._currentReportName == controlObj._clientReportCollection[i].name)) {
                                controlObj._fieldSelectedMembers[item.Tag] = controlObj._clientReportCollection[i]._fieldSelectedMembers[item.DimensionName];
                                filterState = (controlObj._fieldSelectedMembers[item.Tag] == "All" || controlObj._fieldSelectedMembers[item.Tag] == undefined ? controlObj._getLocalizedLabels("All") : controlObj._fieldSelectedMembers[item.Tag]);
                                break;
                            }
                            else
        ...
        _getInitialSlicerName: function (customArgs, args) {
            customArgs.controlObject._slicerName = $($(args).find("Axis:eq(0) Tuple:eq(0)").children().children()[1]).text();
        },
