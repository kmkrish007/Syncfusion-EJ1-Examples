    ...
    ej.PivotAnalysis = {
                ...
                for (var cnt = 0; cnt < dataSource.data.length; cnt++) {
                    ...
                    for (var nC = 0; nC < calcValues.length; nC++) {
                        ...
                        if ($.isNumeric(outputString) || calcValues[nC].format == "date")
                            values.keys.push((val != null ? val : null));
                        else {
                            values.keys.push(1);
                            if (isLiteral) {
            ...
            for (var rCnt = 0; rCnt < (this._tRowCnt - 1) ; rCnt++) {
                for (var cCnt = 0; cCnt < (this._tColCnt - 1) ; cCnt++) {
                    ...
                    if (this._gridMatrix[rCnt] != undefined && this._gridMatrix[rCnt][cCnt] != undefined) {
                        var value = ((typeof (this._gridMatrix[rCnt][cCnt]) === 'string' || typeof (this._gridMatrix[rCnt][cCnt]) === 'number' || typeof (this._gridMatrix[rCnt][cCnt]) === 'object') ? this._gridMatrix[rCnt][cCnt].Value == 0 ?  this._gridMatrix[rCnt][cCnt].Value : this._gridMatrix[rCnt][cCnt].Value == undefined ? "" : !ej.isNullOrUndefined(this._gridMatrix[rCnt][cCnt].valueText) ? this._gridMatrix[rCnt][cCnt].valueText : this._gridMatrix[rCnt][cCnt].Value : "");
        ...
        _applyValueFilter: function (filterInfo, filterValue) {
            ...
            if (filterInfo.length > 0 && !ej.isNullOrUndefined(filterValue)) {
                filterValue = this._isNumber(filterValue) ? filterValue : ej.parseFloat(filterValue, this._locale);
        ...
        _toAccounting: function (value, formatStr, locale) {
            ...
            val = ej.widgetBase.formatting(formatStr, value, locale);
        ...
        _getSummaryValue: function (oldValue, newValue, count, type, format, formatString) {
            newValue = ej.isNullOrUndefined(newValue) ? null : (newValue == 0 ? 0 : newValue);
            oldValue = ej.isNullOrUndefined(oldValue) ? null : (oldValue == 0 ? 0 : oldValue);
            if (!ej.isNullOrUndefined(newValue) && !ej.isNullOrUndefined(oldValue)) {
                ...
                for (var cCnt = 0; cCnt < this._colKeysCalcValues.length; cCnt++) {
                    ...
                    if (this._colKeysCalcValues[cCnt].cellType != undefined && (this._colKeysCalcValues[cCnt].cellType.indexOf("SubTot") > -1 || this._colKeysCalcValues[cCnt].cellType.indexOf("RGTot") > -1)) {
                        if (!this._isPaging) {
                            ...
                            if (!ej.isNullOrUndefined(calValue)) {
