...
(function ($, ej, undefined) {
    ...
    ej.widget("ejFileExplorer", "ej.FileExplorer", {
        ...
        _readSuccess: function (result) {
            ...
            for (var j = 0; j < this.model.gridSettings.columns.length; j++) {
                if (this.model.gridSettings.columns[j]['format']) {
                    for (var k = 0; k < result.length; k++) {
                        result[k]["dateModified"] = new Date(result[k]["dateModified"]);
                    }
                }
            }
        ...
        _renderGridView: function (result) {
            ...
            for (var i = 0; i < columnSettings.length; i++) {
                if (!columnSettings[i]["template"] && !columnSettings[i]["format"]) {
        ...
        _unselectEvent : function(){
            ...
            if (prevSelecteditems.length > 0 ) {
                ...
                for (var record = 0; record < prevSelecteditems.length; record++) {
                    var name = prevSelecteditems[record].name;
        ...
        _OnKeyDown: function (e) {
            ...
            switch (code) {
                case 38:
                    ...
                    if (this._activeItem < this._perRow)
                        return;
                ...
                case 40:
                    e.preventDefault();
                    if(this._activeItem == this._itemList.length - 1){
                        return;
                    }
                    else{
                        if (this._activeItem + this._perRow >= this._itemList.length) {
                            if(this.items[this._activeItem].getBoundingClientRect().top == this.items[this._itemList.length - 1].getBoundingClientRect().top){
                                return;
                            }
                            else{
                                this._activeItem = this._itemList.length - 1;
                            }
                        }
                        else {
                            this._activeItem += this._perRow;
        ...
        _uploadFile: function () {
           var proxy = this.element.find(".e-uploadbox");
        ...
        _sortingActioncomplete: function () {
            ...
            if (this._gridObj && this._gridObj._scrollObject ) {
                this._gridObj._scrollObject.refresh();
            }
        },
        ...
        _validateKeyCode: function (event) {
            switch (event.which) {
                ...
                case 27:
                    this._searchbar && this._searchbar.val("");
                    this._queryString = "";
                    var address = this._currentPath.replace(this._initPath, "");
                    if (this.model.rootFolderName.length > 0)
