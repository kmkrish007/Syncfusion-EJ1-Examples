         ...
         _getExportModel: function (modelClone) {
            ...
            for (var i = 0; i < modelClone.columns.length; i++) {
                if (!ej.isNullOrUndefined(modelClone.columns[i].template)) {
					if ($.isFunction(modelClone.columns[i].template)) {
						var template = modelClone.columns[i].template();
						modelClone.columns[i].template = escape(template);
					}
                    else if (modelClone.columns[i].template.indexOf("#") != -1) {
