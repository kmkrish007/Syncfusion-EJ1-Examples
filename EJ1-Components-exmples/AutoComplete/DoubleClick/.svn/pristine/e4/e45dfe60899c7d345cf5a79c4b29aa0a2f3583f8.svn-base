...
(function ($, ej, undefined) {
    ...
    ej.widget("ejPivotSchemaDesigner", "ej.PivotSchemaDesigner", {
        ...
        _contextClick: function (args) {
             if ($(args.element).hasClass("summarytype")) {
                var selectedValue = args.element.id;
                if (selectedValue != null) {
                    var summary = selectedValue.toLowerCase();
                    var pivotClient = this.model.pivotControl;
                    if (pivotClient.model.analysisMode == ej.Pivot.AnalysisMode.Pivot && pivotClient.model.operationalMode == ej.Pivot.OperationalMode.ClientMode) {
                        if (pivotClient.model.dataSource.values.length >= 1) {
                            for (i = 0 ; i < pivotClient.model.dataSource.values.length; i++) {
                                var selectedValue = this._selectedMember[0].dataset.fieldname
                                 if(pivotClient.model.dataSource.values[i].fieldName == selectedValue)
                                 pivotClient.model.dataSource.values[i].summaryType = summary;
                            }
                            pivotClient.refreshControl();
                        }
                    }
                    else if (pivotClient.model.analysisMode == ej.Pivot.AnalysisMode.Pivot && pivotClient.model.operationalMode == ej.Pivot.OperationalMode.ServerMode) {
                        if (JSON.parse(pivotClient.getOlapReport()).PivotCalculations.length >= 1) 
                            {
                                var selectedValue = this._selectedMember[0].dataset.fieldname
                                var value = args.element.id;
                                var summaryInfo = { "fieldname": selectedValue, "type": value };
                                pivotClient.doAjaxPost("POST", pivotClient.model.url + "/" + pivotClient.model.serviceMethodSettings.changeSummaryType, JSON.stringify({
                                    "action": "changeSummaryType", "currentReport": JSON.parse(pivotClient.getOlapReport()).Report, "summaryType": JSON.stringify(summaryInfo), "clientParams": pivotClient.model.enableMeasureGroups + "-" + pivotClient.model.chartType + "-" + pivotClient.model.enableKPI
        ...
        _createTreeView: function (args, dataSourceInfo) {
            ...
            if (this.model.pivotControl.model.operationalMode == ej.PivotGrid.OperationalMode.ServerMode && this.model.pivotControl.model.analysisMode == ej.Pivot.AnalysisMode.Olap) {
                var report = JSON.parse(this.model.pivotControl.getOlapReport());
				for (var i = 0; i < report.PivotRows.length; i++) {
					if (report.PivotRows[i].FieldHeader != "Measures" && report.PivotRows[i].Filtered) {
						filterCollection.push(report.PivotRows[i].FieldHeader);
					}
				}
				for (var i = 0; i < report.PivotColumns.length; i++) {
					if (report.PivotColumns[i].FieldHeader != "Measures" && report.PivotColumns[i].Filtered) {
						filterCollection.push(report.PivotColumns[i].FieldHeader);
					}
				}
				for (var i = 0; i < report.Filters.length; i++) {
					if (report.Filters[i].FieldHeader != "Measures" && report.Filters[i].Filtered) {
						filterCollection.push(report.Filters[i].FieldHeader);
					}
				}
            }
            ...
            for (var i = 0; i < treeViewElements.length; i++) {
                ...
                if ($(checkedBoxNode[0]).attr("aria-checked") == "true" && $($(treeViewElements[i])).find(".e-folderCDB").length <= 0 && $(treeViewElements[i]).attr("data-tag").toLowerCase().indexOf("[measures]") == -1) {
                    if (this.model.pivotControl._dataModel == "XMLA") {
                        ...
                        var dropSpan = ej.buildTag("span.e-icon").css("display", "none").addClass("treeDrop")[0].outerHTML;
                        $($(treeViewElements[i]).find(".e-text")[0]).after(dropSpan);
						if ($.inArray($($(treeViewElements[i]).find(".e-text")[0])[0].innerText, filterCollection) > -1 && $(treeViewElements[i]).find(".filter").length == 0) {
                            var filterSpan = ej.buildTag("span.e-icon").addClass("filter")[0].outerHTML;
                            $($(treeViewElements[i]).find(".e-text")[0]).after(filterSpan);
                        }
                    }
                }
            }
       ...
       _nodeDraged: function (args) {
           ...
           var minusIcon = (!(ej.isNullOrUndefined(controlObj)) && (controlObj.model.operationalMode == "clientmode" && (!(args.target.hasClass("e-schemaValue")) && !(args.target.parents().hasClass("e-schemaValue")))) || (controlObj.model.operationalMode == "servermode" && !(args.target.hasClass("e-schemaRow") || args.target.hasClass("e-schemaColumn")) && !(args.target.parents().hasClass("e-schemaRow") || args.target.parents().hasClass("e-schemaColumn")))) && ($(".pivotTreeViewDragedNode .e-dropedStatus").hasClass("e-plus")) && (args.draggedElementData.id.indexOf("Measures") >= 0);
            if (args.draggedElementData.id != null && controlObj.model.analysisMode == ej.Pivot.AnalysisMode.Olap && controlObj.model.operationalMode == "clientmode") {
            if (args.draggedElementData.id.indexOf("Measures") > 0) {
                    minusIcon =(!(args.target.hasClass("e-schemaValue"))) && (!(args.target.parents().hasClass("e-schemaValue")));
                }
                else {
                   minusIcon =  (args.target.hasClass("e-schemaValue") || (args.target.parents().hasClass("e-schemaValue")));
                }
        ...
        _wireEvents: function () {
          ...
          .mouseout(ej.proxy(function (evt) {
            ...
            this._on(this.element, "click", ".e-summarytype", ej.proxy(this._contextClick, this));
        },
        ...
        _unWireEvents: function () {
            ...
            this._off(this.element, "click", ".e-summarytype");
        },
        ...
        _createPivotButtons: function (axis, items) {
            ...
            if (!ej.isNullOrUndefined(this.model.pivotControl) && this.model.pivotControl.model.operationalMode == ej.Pivot.OperationalMode.ClientMode) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    if (axis == "values" && this.element.parents(".e-pivotclient").length != 0) {
                        if (items[i].summaryType != null) {
                             var fieldcaption = fldName ||fldCaption;
                            if (items[i].summaryType == "sum")
                                fldName = this._getLocalizedLabels("Sum") + " " + this._getLocalizedLabels("SummaryOf") +  " " + fieldcaption;
                            else if (items[i].summaryType == "average")
                                fldName = this._getLocalizedLabels("Average") + " " + this._getLocalizedLabels("SummaryOf") + " " + fieldcaption;
                            else if (items[i].summaryType == "count")
                                fldName = this._getLocalizedLabels("Count") + " " + this._getLocalizedLabels("SummaryOf") + " " + fieldcaption;
                            else if (items[i].summaryType == "min")
                                fldName = this._getLocalizedLabels("Min") + " " + this._getLocalizedLabels("SummaryOf") + " " + fieldcaption;
                            else if (items[i].summaryType == "max")
                                fldName = this._getLocalizedLabels("Max") + " " + this._getLocalizedLabels("SummaryOf") + " " + fieldcaption;
                        }
                         for (var k = 0; k < items.length; k++) {
                            for (var j = 0; j < $(this.element).find("li").length; j++) {
                                if ($(this.element).find("li")[j].textContent == items[k].fieldCaption){
                              $(this.element.find("li")[j]).attr("summaryType", items[k].summaryType)  
                              }
                              }
                        }
                        if (items[i].summaryType == null) {
                                fldName = this._getLocalizedLabels("Sum") + " " + this._getLocalizedLabels("SummaryOf") +  " " + fldName;
                        }
                ...
                if (axis == "values") {
                  ...
                  if(this.element.parents(".e-pivotclient").length != 0){
                    for (i = 0; i < items.length; i++) {
                        if (items[i].SummaryType != "null") {
                            if (items[i].SummaryType == "Sum")
                                items[i].FieldHeader = this._getLocalizedLabels("Sum") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "Average")
                                items[i].FieldHeader = this._getLocalizedLabels("Average") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "Count")
                                items[i].FieldHeader = this._getLocalizedLabels("Count") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "Min")
                                items[i].FieldHeader = this._getLocalizedLabels("Min") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "Max")
                                items[i].FieldHeader = this._getLocalizedLabels("Max") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DoubleTotalSum")
                                items[i].FieldHeader = this._getLocalizedLabels("DoubleSum") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DoubleAverage")
                                items[i].FieldHeader = this._getLocalizedLabels("DoubleAverage") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DoubleMinimum")
                                items[i].FieldHeader = this._getLocalizedLabels("DoubleMin") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DoubleMaximum")
                                items[i].FieldHeader = this._getLocalizedLabels("DoubleMax") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DoubleStandardDeviation")
                                items[i].FieldHeader = this._getLocalizedLabels("DoubleStandardDeviation") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DoubleVariance")
                                items[i].FieldHeader = this._getLocalizedLabels("DoubleVariance") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "DecimalTotalSum")
                                items[i].FieldHeader = this._getLocalizedLabels("DecimalSum") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "IntTotalSum")
                                items[i].FieldHeader = this._getLocalizedLabels("IntSum") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "CountNumbers")
                                items[i].FieldHeader = this._getLocalizedLabels("CountNumbers") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "StdDev")
                                items[i].FieldHeader = this._getLocalizedLabels("StdDev") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "StdDevP")
                                items[i].FieldHeader = this._getLocalizedLabels("StdDevP") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "Var")
                                items[i].FieldHeader = this._getLocalizedLabels("Variance") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            else if (items[i].SummaryType == "VarP")
                                items[i].FieldHeader = this._getLocalizedLabels("VarP") + " " + this._getLocalizedLabels("SummaryOf") + " " + items[i].FieldHeader;
                            }         
                        }
                    }
                }
        ...
        _pvtBtnDropped: function (args) {
            ...
            if ($(this.element).parents(".e-pivotclient").length > 0 && this.model.pivotControl.model.analysisMode == ej.Pivot.AnalysisMode.Pivot) {
                ...
                var headerText = args.element.attr("data-fieldCaption") || args.element.attr("data-fieldName"); 
                if (args.element.parents(".e-schemaValue").length > 0) {
                    headerText = args.element.attr("data-fieldName"); 
                }
                var report;
        ...
        _dialogBtnClick: function (args) {
            ...
            if (this.model.pivotControl.model.operationalMode == ej.PivotGrid.OperationalMode.ClientMode) {
                ...
                if (args.model.text.toLowerCase() != "cancel") {
                    ...
                    var text = this._curFilteredText, textArr = new Array(), obj = {}, filtertext = new Array();
                    ...
                    if (this.model.pivotControl.model.analysisMode == ej.Pivot.AnalysisMode.Pivot) {
                        ...
                        for (var i = 0; i < uncheckedNodes.length; i++) {
                            ...
                            else if (uncheckedNodes[i].checkedStatus == false)
                                textArr.push(uncheckedNodes[i].key || uncheckedNodes[i].name);
                            else
                                filtertext.push(uncheckedNodes[i].key || uncheckedNodes[i].name);
                        }
                        obj[text] = textArr;
                        this.model.pivotControl._fieldSelectedMembers[text] = filtertext.length == 1 ? filtertext[0] : this.model.pivotControl._getLocalizedLabels("MultipleItems");
                        if (!ej.isNullOrUndefined(pivotClientObj)){                        
                        ...
                        pivotClientObj._clientReportCollection = $.map(pivotClientObj._clientReportCollection, function (obj, index) {
                            if (obj.name == reportListData.getValue())
                                obj._fieldSelectedMembers = pivotClientObj._fieldSelectedMembers;
                            return obj;
                        });
                        }
        ...
        _pvtNodeDroppedSuccess: function (report) {
            ...
            if (report[0] != undefined) {
                ...
                if (report.d[2] != null && report.d[2] != undefined)
                    this.model.customObject = report.d[2].Value;
                var keyString = report.d[0].Key;
                if (keyString == "PivotReport")
            ...
            if (!this.model.pivotControl.model.enableDeferUpdate) {
                if (this.model.pivotControl.model.analysisMode == ej.Pivot.AnalysisMode.Pivot && this.model.pivotControl.model.operationalMode == ej.Pivot.OperationalMode.ServerMode) {
                    var pivotclientObj = this.model.pivotControl;
                  if (!ej.isNullOrUndefined(pivotclientObj._clientReportCollection)) {
                    for (var i = 0; i < pivotclientObj._clientReportCollection.length; i++) {
                        if (pivotclientObj._clientReportCollection[i].fieldSelectedMembers != undefined && pivotclientObj.element.find("#reportList")[0].value == pivotclientObj._clientReportCollection[i].name)
                            pivotclientObj._clientReportCollection[i].fieldSelectedMembers = {};
                       }
                    }
                }
                if ($(this.element).parents(".e-pivotclient").length > 0 && this.model.pivotControl.model.analysisMode == ej.Pivot.AnalysisMode.Pivot) {
        ...
        _filterElementSuccess: function (report) {
            ...
            if ($(this.element).parents(".e-pivotclient").length > 0) {
                if (!ej.isNullOrUndefined(report.d)) {
                    ...
                    var colHiddenCells = ($.map(report.d, function (item) { var keyString = item.Key; if (keyString == "FilteredColumnHeaders") return item.Value; }));
                    if (colHiddenCells.length > 0)
                        tempObj["FilteredColumnHeaders"] = colHiddenCells[0];
                    var rowHiddenCells = ($.map(report.d, function (item) { var keyString = item.Key; if (keyString == "FilteredRowHeaders") return item.Value; }));
            ...
            if (this.model.pivotControl.model.enableDeferUpdate) {
                ...
                if (!ej.isNullOrUndefined(args[0]) && args.length > 0) {
                    ...
                    if ($(this.element).parents(".e-pivotclient").length > 0 && this.model.pivotControl.model.operationalMode == ej.Pivot.OperationalMode.ServerMode && this.model.pivotControl.model.analysisMode == ej.Pivot.AnalysisMode.Pivot)
                        report = JSON.parse($.grep(args.d, function (item) { var keyString = item.Key; return keyString == "PivotReport" })[0].Value);
                    else
        ...
        _createContextMenu: function () {
            ...
            var seperator = ej.buildTag("li.e-separator")[0].outerHTML;
            var args = this.model.pivotControl;
             var clientlayout = seperator + "<li id=addsummary ><a>" + this._getLocalizedLabels("SummarizeValueBy") + "</a><ul><li id=sum class=summarytype><a>" + this._getLocalizedLabels("Sum") + "</a><li><li id=average  class=summarytype ><a>" + this._getLocalizedLabels("Average") + "</a><li><li id=count  class=summarytype ><a>" + this._getLocalizedLabels("Count") + "</a><li><li id=min  class=summarytype><a>" + this._getLocalizedLabels("Min") + "</a><li><li id=max  class=summarytype ><a>" + this._getLocalizedLabels("Max") + "</a><li></ul></li>";
             var serverlayout = seperator + "<li id=addsummary ><a>" + this._getLocalizedLabels("SummarizeValueBy") + "</a><ul><li id=Sum  class=summarytype><a>" + this._getLocalizedLabels("Sum") + "</a><li><li id=Average  class=summarytype ><a>" + this._getLocalizedLabels("Average") + "</a><li><li id=Count  class=summarytype ><a>" + this._getLocalizedLabels("Count") + "</a><li><li id=Min  class=summarytype><a>" + this._getLocalizedLabels("Min") + "</a><li><li id=Max  class=summarytype ><a>" + this._getLocalizedLabels("Max") + "</a><li><li id=Doubletotalsum  class=summarytype><a>" + this._getLocalizedLabels("DoubleSum") + "</a><li><li id=Doubleaverage  class=summarytype ><a>" + this._getLocalizedLabels("DoubleAverage") + "</a><li><li id=Doublemin  class=summarytype><a>" + this._getLocalizedLabels("DoubleMin") + "</a><li><li id=Doublemax  class=summarytype ><a>" + this._getLocalizedLabels("DoubleMax") + "</a><li><li id=Deviation  class=summarytype ><a>" + this._getLocalizedLabels("DoubleStandardDeviation") + "</a><li><li id=Doublevariance  class=summarytype ><a>" + this._getLocalizedLabels("DoubleVariance") + "</a><li><li id=Decimalsum  class=summarytype ><a>" + this._getLocalizedLabels("DecimalSum") + "</a><li><li id=Inttotalsum  class=summarytype ><a>" + this._getLocalizedLabels("IntSum") + "</a><li><li id=CountNumbers  class=summarytype ><a>" + this._getLocalizedLabels("CountNumbers") + "</a><li><li id=StdDev class=summarytype ><a>" + this._getLocalizedLabels("StdDev") + "</a><li><li id=StdDevP class=summarytype ><a>" + this._getLocalizedLabels("StdDevP") + "</a><li><li id=Var class=summarytype ><a>" + this._getLocalizedLabels("Variance") + "</a><li><li id=VarP class=summarytype ><a>" + this._getLocalizedLabels("VarP") + "</a><li></ul></li>";
             var summarylayout = !ej.isNullOrUndefined(args) ? (args.model.analysisMode == ej.Pivot.AnalysisMode.Olap || this.element.parents(".e-pivotclient").length == 0 ? " " : args.model.operationalMode == ej.Pivot.OperationalMode.ClientMode && this.element.parents(".e-pivotclient").length != 0 ? clientlayout : this.element.parents(".e-pivotclient").length != 0 ? serverlayout : "") : "";
             var ContextMenuWidth = !ej.isNullOrUndefined(args) ? (args.model.analysisMode == ej.Pivot.AnalysisMode.Pivot && args.locale() == "en-US" ? "180px" : "auto") : "auto";
