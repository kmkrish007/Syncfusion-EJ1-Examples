...
(function ($, ej, undefined) {
    ...
    ej.widget("ejGantt", "ej.Gantt", {
        ...
        _splitRangeCollection: function (ranges, fromField, toField, isForHistogram) {
            ...
            switch (scheduleMode) {
                ...
                case ej.Gantt.ScheduleHeaderType.QuarterYear:
                case ej.Gantt.ScheduleHeaderType.HalfYear:
                    if (isForHistogram) {
        ...
        _updateResourceData: function (data, args) {
            var proxy = this, dataSource = proxy.dataSource(),
                model = proxy.model,
                index = -1;
            if (data.eResourceTaskType == "groupTask" || (data.eResourceTaskType == "resourceTask" && ej.isNullOrUndefined(data.parentItem)) || data.eResourceTaskType == "unassignedTask") {
                if (dataSource instanceof ej.DataManager) {
                    if (dataSource.dataSource.offline && dataSource.dataSource.json) {
                        ...
                        if (proxy._jsonData.indexOf(data.item) != -1);
                        proxy._jsonData.splice(proxy._jsonData.indexOf(data.item), 1);
                    }
                } else {
        ...
        updateScheduleDates: function (startDate, endDate) {
            ...
            if (model.viewType == "histogramView") {
                proxy._updateHistoData();
            }
            ...
            else if (scheduleHeaderType == "year" || scheduleHeaderType == "quarterYear" || scheduleHeaderType == "halfYear") {
        ...
        reRenderChart: function (scheduleMode) {
            ...
            switch (scheduleMode) {
                ...
                case ej.Gantt.ScheduleHeaderType.QuarterYear:
                case ej.Gantt.ScheduleHeaderType.HalfYear:
                    model.scheduleHeaderSettings.scheduleHeaderType = scheduleMode;
                    proxy._perDayWidth = model.perDayWidth;
        ...
        _initPrivateProperties: function () {
            ...
            proxy._quarterYearTexts = null;
            proxy._halfYearTexts = null;
        ...
        _timelineWidthCalculation: function (scaleSize) {
            ...
            else if (scheduleMode == "quarterYear") {
                proxy._perDayWidth = !fitTimescaleSize ? Math.round(timescaleSizeValue * 0.01) : 1;
                proxy._perMonthWidth = proxy._perDayWidth * 30;
                proxy._perMinuteWidth = proxy._perDayWidth / (24 * 60);
            }
            else if (scheduleMode == "halfYear") {
                proxy._perDayWidth = !fitTimescaleSize ? Math.round(timescaleSizeValue * 0.005) : 0.5;
                proxy._perMonthWidth = proxy._perDayWidth * 30;
                proxy._perMinuteWidth = proxy._perDayWidth / (24 * 60);
            }
        ...
        _getTaskLeft: function (currentDate, isMilestone) {
            ...
            if (headerType == ej.Gantt.ScheduleHeaderType.Week || headerType == ej.Gantt.ScheduleHeaderType.Month
                || headerType == ej.Gantt.ScheduleHeaderType.Year || headerType == ej.Gantt.ScheduleHeaderType.QuarterYear || headerType == ej.Gantt.ScheduleHeaderType.HalfYear) {
        ...
        //Method to get total non working times between two date values
        _getNonworkingTime: function (sDate, eDate, isAutoSchedule, isCheckTimeZone) {
            isCheckTimeZone = ej.isNullOrUndefined(isCheckTimeZone) ? true : isCheckTimeZone;
            var timeDiff = this._getTimeDiff(sDate, eDate, isCheckTimeZone)/ 1000, // To convert milli seconds to seconds
              weekendCount = !this.model.includeWeekend && isAutoSchedule ? this._getWeekendCount(sDate, eDate) : 0,
              totalHours = this._getNumberOfSeconds(sDate, eDate, isCheckTimeZone),
              holidaysCount = isAutoSchedule ? this._getHolidaysCount(sDate, eDate) : 0;
            var totalWorkingDays = (totalHours - (weekendCount * 86400) - (holidaysCount * 86400)) / 86400; // working days between two dates
            var nonWorkingHours = this._getNonWorkingSecondsOnDate(sDate, eDate, isAutoSchedule);
            var totalNonWorkTime = (totalWorkingDays * (86400 - this._secondsPerDay)) + (weekendCount * 86400) + (holidaysCount * 86400) + nonWorkingHours;
        ...
        _getEndDate: function (startDate, duration, durationUnit, record, validateAsMilestone) {
            var secondDuration = 0, tempStart = new Date(startDate), endDate = new Date(startDate);
            ...
            while (secondDuration > 0) {
                ...
                nonWorkingHours = this._getNonworkingTime(tempStart, endDate, record.isAutoSchedule, true);
                secondDuration = nonWorkingHours;
                if (secondDuration > 0)
                    endDate = this._checkStartDate(endDate, record, validateAsMilestone);
                tempStart = new Date(endDate);
            }
            return endDate;
        },
        //Calculate start date with endDate,duration ,duration Unit
        _getStartDate: function (endDate, duration, durationUnit, record) {
            var secondDuration = 0, tempEnd = new Date(endDate), startDate = new Date(endDate);
            if (!durationUnit || durationUnit == ej.Gantt.DurationUnit.Day)
                secondDuration = this._secondsPerDay * duration;
            else if (durationUnit == ej.Gantt.DurationUnit.Hour)
                secondDuration = duration * 3600;
            ...
            while (secondDuration > 0) {
                ...
                nonWorkingHours = this._getNonworkingTime(startDate, tempEnd, record.isAutoSchedule, true);
                secondDuration = nonWorkingHours;
                if (secondDuration > 0)
                    tempEnd = this._checkEndDate(startDate, record);
                tempEnd = new Date(startDate);
            }
            return startDate;
        },
        ...
        _getDuration: function (sDate, eDate, durationUnit, isAutoSchedule, isCheckTimeZone) {
            if (ej.isNullOrUndefined(sDate) || ej.isNullOrUndefined(eDate))
                return null;
            isCheckTimeZone = ej.isNullOrUndefined(isCheckTimeZone) ? true : isCheckTimeZone;
            var timeDiff = this._getTimeDiff(sDate, eDate, isCheckTimeZone) / 1000,
                durationValue,
                nonWorkingHours = this._getNonworkingTime(sDate, eDate, isAutoSchedule, isCheckTimeZone),
            durationHrs = timeDiff - nonWorkingHours;
            if (!durationUnit || durationUnit == ej.Gantt.DurationUnit.Day) {
        ...
        _getNonWorkingSecondsOnDate: function (startDate, endDate, isAutoSchedule) {
            var proxy = this, sHour = this._getSecondsInDecimal(startDate),
                eHour = this._getSecondsInDecimal(endDate),
                startRangeIndex = -1,
                endRangeIndex = -1,
                totNonWrkSecs = 0,
                startOnHoliday = isAutoSchedule ? this._isOnHolidayOrWeekEnd(startDate) : false,
                endOnHoliday = isAutoSchedule ? this._isOnHolidayOrWeekEnd(endDate) : false;

            for (var i = 0; i < this._nonWorkingTimeRanges.length; i++) {
                var val = this._nonWorkingTimeRanges[i];
                if (sHour >= val.from && sHour <= val.to)
                    startRangeIndex = i;
            ...
            if (startDate.getDate() != endDate.getDate() || startDate.getMonth() != endDate.getMonth() || startDate.getFullYear() != endDate.getFullYear()) {
                ...
                if (!startOnHoliday) {
                    for (var i = startRangeIndex; i < this._nonWorkingTimeRanges.length; i++) {
                        if (!this._nonWorkingTimeRanges[i].isWorking) {
                            if (i == startRangeIndex) {
                                totNonWrkSecs += (this._nonWorkingTimeRanges[i].to - sHour);
                            } else {
                                totNonWrkSecs += (this._nonWorkingTimeRanges[i].interval);
                            }
                        }
                    }
                } else {
                ...
                if (!endOnHoliday) {
                    for (var i = 0; i <= endRangeIndex; i++) {
                        if (!this._nonWorkingTimeRanges[i].isWorking) {
                            if (i == endRangeIndex) {
                                totNonWrkSecs += (eHour - this._nonWorkingTimeRanges[i].from);
                            } else {
                                totNonWrkSecs += this._nonWorkingTimeRanges[i].interval;
                            }
                        }
                ...
                if (startRangeIndex != endRangeIndex) {
                    ...
                    if (!endOnHoliday) {
                        var range = this._nonWorkingTimeRanges[startRangeIndex];
                        if (!range.isWorking) {
                            totNonWrkSecs = eHour - sHour;
                        }
                    } else {
                ...
                if (currentRange.from && currentRange.to) {
                    ...
                    if (count > 0){
                        ...
                        if (proxy._workingTimeRanges[count - 1].to < sdHour)
                            proxy._nonWorkingTimeRanges.push({ from: proxy._workingTimeRanges[count - 1].to, to: sdHour, isWorking: false, interval: (sdHour - proxy._workingTimeRanges[count - 1].to) });
                    }
                    else{
                        proxy._nonWorkingHours.push(0); 
                        proxy._nonWorkingTimeRanges.push({ from: 0, to: sdHour, isWorking: false, interval: sdHour });
                    }
                    proxy._workingTimeRanges.push({ from: sdHour, to: edHour });
                    proxy._nonWorkingTimeRanges.push({ from: sdHour, to: edHour, isWorking: true, color: currentRange.background, interval: (edHour - sdHour) });
                }
            }
            if (proxy._defaultEndTime / 3600 != 24)
                proxy._nonWorkingTimeRanges.push({ from: proxy._defaultEndTime, to: 86400, isWorking: false, interval: (86400 - proxy._defaultEndTime) });
            return totalSeconds;
        },
        ...
        _setCultureInfo:function() {
            ...
            // QuarterYear schedule texts
            proxy._quarterYearTexts = (localization && localization["quarterYearTexts"]) ?
            localization["quarterYearTexts"] : defaultLocalization["quarterYearTexts"];

            // HalfYear schedule texts
            proxy._halfYearTexts = (localization && localization["halfYearTexts"]) ?
            localization["halfYearTexts"] : defaultLocalization["halfYearTexts"];
        },
        ...
        _renderTreeGrid: function() {
            ...
            proxy._$treegridHelper.ejTreeGrid({
                ...
                dataManagerUpdate: { isDataManagerUpdate: proxy._isDataManagerUpdate, jsonData: [] },
        ...
        _updateScheduleDatesByTaskLables: function (startDate, endDate)
        {
            ...
            if (scheduleHeaderType == scheduleHeaderValue.Year || scheduleHeaderType == scheduleHeaderValue.QuarterYear || scheduleHeaderType == scheduleHeaderValue.HalfYear) {
                //if (model.showTaskNames) {
        ...
        _calculateHeaderDates: function () {
            ...
            if (headerType == headerValue.Year || headerType == headerValue.QuarterYear || headerType == headerValue.HalfYear) {
        ...
        "export": function (action, serverEvent, multipleExport) {
            ...
            if (ej.raiseWebFormsServerEvents) {
                ...
                var locale = { "durationText": proxy._durationUnitTexts, "quarterYearTexts": proxy._quarterYearTexts, "halfYearTexts": proxy._halfYearTexts, "durationEditText": proxy._durationUnitEditText, "taskTypeTexts": proxy._taskTypeTexts, "effortDrivenTexts": proxy._effortDrivenTexts, "taskModeTexts": proxy._taskModeTexts, "nullText": proxy._nullText };
        ...
        _getExportModel: function (ganttModel) {
            var proxy = this, model = ganttModel,
                ...
                updatedGanttRecords = model.updatedGanttRecords;
            ...
            delete model.updatedGanttRecords;
            ...
            model.updatedGanttRecords = updatedGanttRecords;
        ...
        _getDateByLeft: function (left) {
            ...
            if (this.model.scheduleHeaderSettings.scheduleHeaderType == ej.Gantt.ScheduleHeaderType.Week
                ...
                || this.model.scheduleHeaderSettings.scheduleHeaderType == ej.Gantt.ScheduleHeaderType.QuarterYear
                || this.model.scheduleHeaderSettings.scheduleHeaderType == ej.Gantt.ScheduleHeaderType.HalfYear) {
        ...
        _getRoundOffStartLeft: function (ganttRecord, isRoundOff) {
            ...
            if (!isRoundOff) {
                if ((headerType == headerValue.Week || headerType == headerValue.Month || headerType == headerValue.Year || headerType == headerValue.QuarterYear || headerType == headerValue.HalfYear)) {
        ...
        taskbarEdited: function (args) {
            ...
            if (args.progressResizing) {
                if (args.previousData.status != args.data.status) {
                    ...
                    if (ganttRecord.parentItem) {
                        ganttRecord._updateParentProgress(ganttRecord.parentItem, model.progressMapping,this);
                    }
        ...
        _validateAutoChildRecords: function (args) {
            ...
            for (var i = 0; i < childItem.length; i++) {
                if (childItem[i].isAutoSchedule && !childItem[i].hasChildRecords && durationDiff > 0) {
                    ...
                    if (isRightMove) {
                        calcEndDate = this._getEndDate(new Date(childItem[i].startDate), durationDiff, "minute", childItem[i]);
                        childItem[i].left = this._getTaskLeft(this._checkStartDate(new Date(calcEndDate)));
                    } else {
                        calcEndDate = this._getStartDate(this._checkEndDate(new Date(childItem[i].startDate)), durationDiff, "minute", childItem[i]);
        ...
        _changeChartSchedule: function (headerType) {
            ...
            switch (headerType) {
                ...
                case ej.Gantt.ScheduleHeaderType.Year:
                    ...
                    break;
                case ej.Gantt.ScheduleHeaderType.QuarterYear:
                    model.scheduleHeaderSettings.scheduleHeaderType = ej.Gantt.ScheduleHeaderType.QuarterYear;
                    proxy._perDayWidth = 1;
                    proxy._perMonthWidth = 30;
                    break;
                case ej.Gantt.ScheduleHeaderType.HalfYear:
                    model.scheduleHeaderSettings.scheduleHeaderType = ej.Gantt.ScheduleHeaderType.HalfYear;
                    proxy._perDayWidth = 0.5;
                    proxy._perMonthWidth = 15;
                    break; 
       ...
       // Set the splitter position based on column index.
        ...
        _updateScrollerBorder: function () {
            var proxy = this,
                gridScrollObj = $("#ejTreeGrid" + this._id + "e-gridcontent").data("ejScroller"),
		        chartObj = $("#ganttviewerbodyContianerejGanttChart" + this._id),
            ...
            chartObj.removeClass("e-borderbox");
			var height = proxy._$ganttchartHelper.ejGanttChart("getViewportHeight");
            if (isGridHScroll && !isChartHScroll) {
                gridElement.removeClass("e-borderbox");
                chartObj.addClass("e-borderbox");
                chartScrollObj.option("height", (height - 18));
                chartObj.height(height - 16);
                proxy._$ganttchartHelper.ejGanttChart("setViewPortHeight");
            }
            else if (!isGridHScroll && isChartHScroll) {
                  gridElement.addClass("e-borderbox");              
                  chartObj.removeClass("e-borderbox");
                  chartScrollObj.option("height", (height));
        ...
        updateRecordByIndex: function (index, data) {
            ...
            if (!ej.isNullOrUndefined(record)) {
                ...
                if (ganttRecord._isScheduledTask() && !ej.isNullOrUndefined(tempData[model.endDateMapping]) && !(tempData[model.startDateMapping] || !ej.isNullOrUndefined(tempData[model.durationMapping]) )
                ) {
                    var endDateType = proxy._getDateFromFormat(tempData[model.endDateMapping]);
                        if (endDateType.getHours() == 0 && this._defaultEndTime != 86400)
                            this._setTime(this._defaultEndTime, endDateType);
                    if (endDateType.getTime() > (!ej.isNullOrUndefined(ganttRecord.startDate) && ganttRecord.startDate.getTime())) {
                        ganttRecord.endDate = this._checkEndDate(endDateType, ganttRecord);
                        ganttRecord._calculateDuration(this);
                    } else {
                        ganttRecord.endDate = this._checkEndDate(endDateType, ganttRecord);
                        ganttRecord.startDate = this._getStartDate(ganttRecord.endDate, ganttRecord.duration, ganttRecord.durationUnit, ganttRecord);
                    }
                }
                ...
                if (proxy._getParentSelectionStatus(data) || data.level == 0) {
                    continue;
                        ...
                        if (data.parentItem.childRecords.length == 0) {
                            ...
                            data.parentItem._updateWorkWithDuration(proxy);
                        }
                        else
                        {
                            if (data.parentItem && data.parentItem.isAutoSchedule)
        ...
        _updateManualParentItem: function (ganttRecord, editmode, isParent) {
            ...
            var proxy = this,
                ...
                prevManualEndDate = parentGanttRecord.manualEndDate, totalDuration = 0, progressValues,
            ...
            for (var count = 0; count < length; count++) {
                ...
                if ((!childGanttRecord.isMilestone || childGanttRecord.hasChildRecords) && childGanttRecord._isScheduledTask()) {
                    progressValues = proxy._getParentProgress(childGanttRecord);
                    totalProgress += progressValues.totalProgress;
                    totalDuration += progressValues.totalDuration;
                }
                else
                    milestoneCount++;
            }
            ...
            parentGanttRecord.totalProgress = totalProgress;
            parentGanttRecord.totalDuration = totalDuration;
        ...
        _getParentProgress: function (childGanttRecord) {
            var durationInDay = 0, progressValues = {};
            switch (childGanttRecord.durationUnit) {
                case "hour":
                    durationInDay = (childGanttRecord.duration / (this._secondsPerDay / 3600));
                    break;
                case "minute":
                    durationInDay = (childGanttRecord.duration / (this._secondsPerDay / 60));
                    break;
                default:
                    durationInDay = childGanttRecord.duration;
            }

            if (childGanttRecord.hasChildRecords) {
                    progressValues.totalProgress = childGanttRecord.totalProgress;
                    progressValues.totalDuration = childGanttRecord.totalDuration;
                }
            else {
                progressValues.totalProgress = childGanttRecord.status * durationInDay;
                progressValues.totalDuration = durationInDay;
            }
        ...
        _updateParentItem: function (ganttRecord, editmode, isParent) {
            ...
            var proxy = this,
                ...
                childRecords = parentGanttRecord.childRecords,
                length = childRecords && childRecords.length, totalDuration = 0, progressValues,
                minStartDate = null, maxEndDate = null,
                milestoneCount = 0, totalProgress = 0, childCompletedWorks = 0;
    ...
    ej.Gantt.Locale["default"] = ej.Gantt.Locale["en-US"] = {
        ...
        //string to display in QuarterYear scheduler
        quarterYearTexts: {
            first: "Q1",
            second: "Q2",
            third: "Q3",
        ...
        //string to display in HalfYear scheduler
        halfYearTexts:{
            first: "H1",
            second: "H2",
        },
    ...
    ej.Gantt.GanttRecord = function () {
        ...
        proxy.totalProgress = 0;
        proxy.totalDuration = 0;
    ...
    ej.Gantt.GanttRecord.prototype = {
        ...
        _updateParentProgress: function (parent, progressMapping,ganttObj) {
            var parentProgress = 0, proxy = ganttObj,
                childRecords = parent.childRecords, totalDuration = 0, progressValues,
            ...
            if (childRecords) {
                for (var i = 0; i < childCount; i++) {
                    if (!childRecords[i].isMilestone || childRecords[i].hasChildRecords) {
                        progressValues = proxy._getParentProgress(childRecords[i]);
                        totalProgress += progressValues.totalProgress;
                        totalDuration += progressValues.totalDuration;
                    }
                    else
                ...
                parentProgress = taskCount > 0 ? Math.round(totalProgress / totalDuration) : 0;
                parent.totalProgress = totalProgress;
            ...
            if (parent.parentItem) {
                this._updateParentProgress(parent.parentItem, progressMapping, proxy);
            }
        },
        ...
        _calculatePredecessor: function (string, durationUnitLabels, defaultDurationUnit, gantt) {
            var collection = [],
                ...
                proxyGantt = gantt, predecessorText, parentGanttRecord, childGanttRecord;
            ...
            if (typeof string === 'string')
            {
                string.split(',').forEach(function (el) {
                    ...
                    parentGanttRecord = proxyGantt._getRecordByTaskId(obj.from);
                    childGanttRecord = proxyGantt._getRecordByTaskId(obj.to);
                    if(parentGanttRecord){
                        if (!parentGanttRecord.hasChildRecords && (childGanttRecord ? !childGanttRecord.hasChildRecords : !ganttRecord.hasChildRecords)) {
