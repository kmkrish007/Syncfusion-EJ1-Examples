    ...
    ej.widget("ejDropDownList", "ej.DropDownList", {
        ...
        _onValueChange: function () {
            ...
            if ((this.value() != this.element.val()) || (this.value() == null && ($.inArray("", this._valueContainer) != -1))) {
				(!ej.isNullOrUndefined($(this.getSelectedItem()).last()[0]))?this._updateSelectedIndex(this.element.val(),$(this.getSelectedItem()).last().attr("data-uid")): this._updateSelectedIndexByValue(this.element.val());
				if (!this._isSingleSelect() ) {
        ...
        _deleteBoxCheck: function (val) {
            ...
            for (var i = 0; i < items.length; i++) {
                if ($(items[i]).attr("data-value") == val) {
                    $(items[i]).remove();
					this._checkedValues.splice(val, 1);
                }
            }
        },
